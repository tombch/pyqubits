...
def command(env, command_args):
    if len(command_args) < 3:
        ...
        if not utils.is_existing_gate(g, env):
            ...
            if not utils.is_existing_state(s, env):
                ...
                if g_num_args > s_object.num_qubits:
                    ...
                    for q_list in qubits:
                        q_list_object = utils.construct_int_list(q_list)
                        if q_list_object == None or not utils.is_valid_qubit_list_of_state(q_list_object, s_object):
                    ...
                    for q in qubits:
                        if not utils.is_valid_qubit_of_state(q, s_object):
