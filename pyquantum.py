...
class QuantumState:
    ...
    def __init__(self, qubits=1, vector=None):
        ...
        if vector != None:
            if vector == 0 or vector == '0' or vector == 'zero':
            ...
            elif vector == 1 or vector == '1' or vector == 'one':
                # The one state is the state |11...11>
                self.__state_vector = np.zeros(self.__num_classical_states) + np.zeros(self.__num_classical_states) * 1j
                self.__state_vector[-1] = 1
            elif isinstance(vector, np.ndarray):
                self.__state_vector = vector
            else:
        ...
        def print_state(state_vector):
            ...
            for i, bin_i in gen_classical_states:
                ...
                if current_amplitude != 0:
                    print(f"{current_amplitude} |{bin_i}>")
        print_state(self.__state_vector)
        
    def dist(self):
        def print_dist(state_vector):
            gen_classical_states = ((i, bin(i)[2:].zfill(self.__num_qubits)) for i in range(self.__num_classical_states))
            for i, bin_i in gen_classical_states:
                print(f"{bin_i}\t{round(abs(state_vector[i])**2, 2)}\t|{'=' * int(round(50 * abs(state_vector[i])**2))}")
        print_dist(self.__state_vector)
    ...
    def measure(self, qubit=1):
        if not (1 <= qubit <= self.__num_qubits):
            pass # TODO: Error
        if (qubit == 1):
        ...
        zero_probability = self.__state_vector.conjugate().transpose() @ measurement_matrix_zero.conjugate().transpose() @ collapsed_zero
        one_probability = self.__state_vector.conjugate().transpose() @ measurement_matrix_one.conjugate().transpose() @ collapsed_one
    ...
    def Uf2(self, qubit1=1, qubit2=2, f=random.randint(1, 4)):
        if f == 1:
            ...
            raise QuantumStateError(f"invalid choice of f: {f}")

        if self.__num_qubits < 2:
            raise QuantumStateError("cannot apply 'Uf2' to a one-qubit state")
        if qubit1 > self.__num_qubits:
            raise QuantumStateError(f"qubit1 reference out of bounds: {qubit1}")
        if qubit2 > self.__num_qubits:
            raise QuantumStateError(f"qubit2 reference out of bounds: {qubit2}")
        if qubit1 + 1 != qubit2:
            raise QuantumStateError("qubit1 must be left of (and next to) qubit2")    
