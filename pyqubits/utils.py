        ...
        for name, value in kwargs.items():
            ...
            if arg_types.get(name) == int:
                if (not isinstance(value, int)) or (not (1 <= value <= obj._num_qubits)):
                    raise PyQubitsError(f"'{name}' must be a positive integer, less than or equal to the number of qubits in the state")
...
memkron(np.asarray([gates.I_matrix]), np.asarray([1.+0.j, 0.+0.j]), np.asarray([[0], [1]]))
